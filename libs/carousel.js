 ...
 *                  init            : launch just to the end of the initialization/loading of the carousel
...
Carousel.prototype = {
        ...
        if (typeof(config.elements) != 'object')
        {
            ...
            for(var i=0; i<elements.length; i++)
            {
                if(elements[i].nodeType == 1)
                    config.elements[id_element++]=elements[i];
            }
        }
    ...
    {        
        ...
        // initialization of the elements array
    ...
    _insertElementIntoContainer: function(id_element) {
        ...
        if( this.config.navigation.type.search('thumbnail') != -1 )
        {
            ...
            {
                // by default, use elements
                if( typeof(this.attributes.elements[id_element].tag) != 'object' )
                    thumbnail.innerHTML = this.attributes.elements[id_element].tag;
                else
                    thumbnail.appendChild(this.attributes.elements[id_element].tag.cloneNode(true));
            }
    ...
    /**
     * Destroy the carousel and restore (if requested) the elements into the container 
     */
    destroy: function(restore_element){
        window.clearTimeout(this.attributes.timer);
        this.attributes.container.removeChild(this.attributes.elementsContainer);
        this.attributes.container.removeChild(this.config.navigation.container);
        
        if( restore_element === true )
        {
            for(i in this.attributes.elements)
                this.attributes.container.appendChild(this.attributes.elements[i].tag);
        }
